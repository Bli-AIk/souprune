...
if [ -n "$mod_files" ]; then
    echo "Error: Found mod.rs files. Use Rust 2018+ module naming instead:"
...
over_limit=$(tokei "$SEARCH_DIR" --output json --files \
    | jq -r --argjson max "$MAX_LINES" \
        '.Rust.reports[]? | select(.stats.code > $max) | "Error: \(.name) has \(.stats.code) lines of code (max \($max))"')
...
if [ "$errors" -ne 0 ]; then
    ...
    echo "Tokei OK: All Rust files under $MAX_LINES lines of code, no mod.rs found."
fi
