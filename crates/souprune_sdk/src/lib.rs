...
macro_rules! declare_soul_mode {
    ($mod_type:ty, $constructor:expr) => {
        ...
        // 方案 B: Mod 有状态。
        //
        // 让我们暂时假设 Mod 是无状态的逻辑控制器 (Controller)，状态都通过 ctx 存取。
        // 如果需要存 Mod 自己的状态，我们需要更复杂的 FFI 握手。
        //
        // 让我们先实现 无状态逻辑 版本。

        #[no_mangle]
        pub extern "C" fn create_soul_mode(
            api: *const soup_api::HostApi,
        ) -> soup_api::SoulModeVTable {
            unsafe {
        ...
        // === 适配器函数 (C ABI -> Rust Trait) ===

        extern "C" fn wrapper_on_update(handle: *mut soup_api::ContextHandle, dt: f32) {
            unsafe {
                if let Some(api) = &HOST_API {
                    // 1. 构建 Safe Context
                    let mut ctx = $crate::Context::new(handle, api);
                    ...
                    // 3. 调用用户逻辑
                    mode.on_update(&mut ctx, dt);
                }
            }
        }

        extern "C" fn wrapper_on_enter(handle: *mut soup_api::ContextHandle) {
            unsafe {
                if let Some(api) = &HOST_API {
                    let mut ctx = $crate::Context::new(handle, api);
                    let mut mode = $constructor();
        ...
        extern "C" fn wrapper_on_exit(handle: *mut soup_api::ContextHandle) {
            unsafe {
                if let Some(api) = &HOST_API {
                    let mut ctx = $crate::Context::new(handle, api);
                    let mut mode = $constructor();
                    mode.on_exit(&mut ctx);
                }
            }
        }
    };
