//! Renders editable `LocalState` views for every active `ViewRoot` in the FRE debug panel.
//!
//! 在 FRE 调试面板里渲染每个活动 `ViewRoot` 的可编辑 `LocalState` 视图。
//!
//! Focuses specifically on per-view local state rather than the shared layered
//! fact database. It lists active view roots, filters their local state values,
//! and lets the debugger tweak values in place so UI logic can be inspected
//! without reloading the whole game state.
//!
//! 只关注每个 View 自己携带的局部状态，而不是全局共享的 layered
//! fact database。它会列出活动中的 view root，过滤其局部状态，并允许调试时
...
/// Render the View LocalState tab.
pub(super) fn render_view_state_tab(ui: &mut egui::Ui, world: &mut World) {
    ...
    egui::ScrollArea::vertical().show(ui, |ui| {
        if view_roots.is_empty() {
            ...
            ui.label("Views with LocalState will appear here when loaded.");
            return;
        }
        ...
        for (entity, key, value) in modifications {
            if let Some(mut view_root) = world.get_mut::<ViewRoot>(entity) {
                view_root.override_local_value_for_debug(key, value);
            }
        }
    });
}
