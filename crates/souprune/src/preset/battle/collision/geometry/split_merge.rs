...
//! ## 模块概述
//!
//! Contains the pure geometry calculations for battle-box splitting and merging. It
...
//!
//! 放的是战斗框分裂与合并的纯几何计算。它会求出一次分裂后产生的矩形边界，推导两个
//! 战斗框合并时的目标形状，并把这些计算和真正驱动 ECS 动画的系统代码分离开来。

use super::*;

/// Split a rectangular boundary along an axis.
pub(in crate::preset::battle::collision) fn split_rect_box(
    original: &CollisionBoundary,
    axis: &SplitAxis,
    split_pos: f32,
    gap: f32,
...
fn infer_merge_axis(a: &CollisionBoundary, b: &CollisionBoundary) -> Option<SplitAxis> {
...
pub(in crate::preset::battle::collision) fn plan_merge_animation(
    boundaries: &[CollisionBoundary; 2],
    gap_policy: GapPolicy,
...
pub(in crate::preset::battle::collision) fn merge_boundaries(
    a: &CollisionBoundary,
    b: &CollisionBoundary,
) -> CollisionBoundary {
    let min = Vec2::new(
    ...
    CollisionBoundary { half_size, center }
}
