...
pub fn chase_damage_detection_system(
    ...
    time: Res<Time>,
    chase_config: Res<ChaseConfig>,
    player_behavior: Res<crate::core::top_down::player::config::PlayerBehavior>,
    ...
    for _event in damage_events.read() {
        if !damage_ui_state.showing {
            let Some(scope) = crate::core::top_down::top_down_scoped(&sequence_mode) else {
                warn!("Chase damage UI: no active mode while spawning damage flash.");
                return;
            };
...
pub(super) fn setup_chase_hud_system(
    ...
    spawn_writer.write(crate::core::view::SpawnViewRequest {
        ...
        mode_scope: Some("top_down".to_string()),
