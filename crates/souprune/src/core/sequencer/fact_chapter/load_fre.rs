//! Loads FRE assets for sequence chapters and merges their facts into runtime state.
//!
//! 为序列章节加载 FRE 资源，并把其中的 facts 合并进运行时状态。
//!
...
//! 资源加载完成，注册枚举，把解析后的 facts 写入 layered 数据库，并按需执行
