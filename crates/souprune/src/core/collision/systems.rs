...
#[cfg(feature = "debug")]
pub fn toggle_collider_visibility(
...
pub fn render_player_rect_colliders(
    ...
    for (entity, transform, collider) in player_query.iter() {
        ...
        // Create the SDF rectangle border shader expression
        // Using a hollow rectangle technique: outer rect minus inner rect
        let border_sdf = shaders.add_sdf_expr(format!(
            "max(smud::sd_box(p, vec2<f32>({}, {})), -smud::sd_box(p, vec2<f32>({}, {})))",
            collider.size.x / 2.0,
            collider.size.y / 2.0,
            collider.size.x / 2.0 - 1.0,
            collider.size.y / 2.0 - 1.0,
        ));

        // Calculate frame size based on collider size
        let frame_size = (collider.size.x.max(collider.size.y) / 2.0) + 5.0;

        // Calculate final position including offset
        let final_position = transform.translation + collider.offset.extend(0.1); // Slightly above to be visible
...
/// 当父变换改变时更新可视化器位置的系统
pub fn update_player_visualizer_positions(
