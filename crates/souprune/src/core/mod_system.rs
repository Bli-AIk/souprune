...
use souprune_api::{CreateSoulModeFn, HostApi, SoulModeVTable, ContextHandle, Action};
...
extern "C" fn host_input_is_action_pressed(_context: *const ContextHandle, action: Action) -> bool {
    INPUT_SNAPSHOT.with(|snapshot| {
        snapshot.borrow().is_pressed(action)
    })
}
...
impl Plugin for ModPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SoulRegistry>()
           ...
           .add_systems(Update, update_souls_system.in_set(crate::app_state::battle::BattleUpdate));
    }
}
...
fn load_mods_system(mut registry: ResMut<SoulRegistry>) {
    ...
    // In production, this should iterate over `projects/example_mod/mods/*.so` or read `mod.toml`
    let lib_name = if cfg!(target_os = "windows") {
        "mod_soul_red.dll"
    } else {
        "libmod_soul_red.so"
    };
    let mod_path = format!("projects/example_mod/mods/{}", lib_name);
    ...
    unsafe {
        ...
        // 1. Get ID
        let get_id: Symbol<extern "C" fn() -> *const i8> = lib.get(b"get_soul_mode_id").expect("No ID found");
        let id = CStr::from_ptr(get_id()).to_string_lossy().into_owned();
        
        // 2. Get Factory
        let create_fn: Symbol<CreateSoulModeFn> = lib.get(b"create_soul_mode").expect("No factory found");
        
        let vtable = create_fn(&HOST_API_INSTANCE);
...
fn update_souls_system(
    mut query: Query<(Entity, &SoulParams, &mut SoulState, &mut SoulVelocity, &mut Transform)>,
    ...
    INPUT_SNAPSHOT.with(|s| {
        ...
        snap.pressed[Action::Cancel as usize] = input.pressed(KeyCode::KeyX) || input.pressed(KeyCode::ShiftLeft);
        snap.pressed[Action::Confirm as usize] = input.pressed(KeyCode::KeyZ) || input.pressed(KeyCode::Enter);
    });
