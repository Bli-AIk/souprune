...
use crate::core::battle_runtime::{
    alight_motion_integration::AlightMotionBattlePlugin, danmaku::DanmakuPlugin,
...
pub(crate) fn on_entering_battle(mut events: MessageReader<ModeChanged>) -> bool {
    events.read().any(|e| e.to.as_deref() == Some("battle"))
}

pub(crate) fn on_exiting_battle(mut events: MessageReader<ModeChanged>) -> bool {
