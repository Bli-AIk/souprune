//! Dynamic View element components.
//!
//! 动态 View 元素组件。
//!
//! This module contains components for dynamic View elements that need runtime updates.
//! The HP bar functionality has been migrated to the new ShaderMaterial + DynamicMaterial2d system.
//!
//! 该模块包含需要运行时更新的动态 View 元素组件。
//! HP 条功能已迁移到新的 ShaderMaterial + DynamicMaterial2d 系统。

use bevy::prelude::*;

/// Marker component for View elements that need dynamic updates based on player data.
...
/// Marker component for View elements whose transform depends on time (@time).
/// These elements need per-frame updates for animation.
///
/// 标记 Transform 依赖时间 (@time) 的 View 元素的组件。
/// 这些元素需要逐帧更新以实现动画。
