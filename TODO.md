...
> 💡 **说明**：框架在 v0.4.1 后将走向项目驱动而非提前设计。
>
> 即“我会在做实际项目/测试demo的过程中，了解框架缺乏的内容并进行版本更新”。
>
> 该状态可能会长期持续，直到“正式版本发布”。
...
- [x] **SDK 层重构 (`souprune_sdk`)**
    ...
    - [x] **修复状态丢失缺陷 (Critical Fix)**:
        - *原因*: 当前实现每帧都在栈上重新创建 Mod 实例 (`let mut mode = $constructor()`)，导致 Mod 无法保存任何状态 (
          如计数器、计时器)。必须将实例生命周期托管到堆上。
        - [x] **ABI 变更**: 修改 `create_behavior` 签名，返回 `(*mut c_void, BehaviorVTable)` 而非仅 `BehaviorVTable`
          。返回的指针指向堆上的 Mod 实例。
        - [x] **VTable 变更**: 修改 `BehaviorVTable` 中的所有函数指针 (如 `on_update`)，增加 `instance: *mut c_void`
          作为第一个参数 (相当于 `self`)。
        ...
        - [x] **资源清理**: 在 `BehaviorVTable` 中增加 `destroy` 函数，用于在 Host 销毁 Behavior 时调用
          `Box::from_raw(instance)` 重新获取所有权并 Drop，防止内存泄漏。
...
- [x] **核心数据结构定义 (数据驱动)**
    - [x] 定义 `DanmakuBlueprint` 资产 (`.danmaku.ron`)
        ...
        - [x] `MotionTrack`: Linear, Orbital (原 Circular), Sine, Aimed (自机狙), Tween, Custom (原 Algo) 运动轨道
...
- [x] **Phase 2: C# 集成 (.NET Native AOT)**
    ...
    - [x] 建立 `souprune-sdk-dotnet` 项目结构 (`crates/souprune_sdk_csharp/`)
...
- [x] **Phase 3: Haxe 集成 (hxcpp)**
    - [x] 在 `bindgen` 中实现 Haxe 代码生成逻辑 (`crates/interoptopus_backend_haxe/`)
    - [x] 建立 `souprune-sdk-haxe` 项目结构 (`crates/souprune_sdk_haxe/`)
    - [x] 实现 Haxe 版本弹幕 Mod (`projects/example_mod/code/mod_example_haxe/`)

---

## 📝 待办清单 (Backlog)

### 多语言 SDK 集成 (Multilingual SDK Integration)
...
### 战斗 UI 与交互 (Battle UI Interaction)
...
### Overworld 机制 (Overworld Systems)
