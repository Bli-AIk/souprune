...
> **框架（framework），不是引擎（engine）**：SoupRune 是*框架*——Bevy 才是底层的游戏引擎。
> 在文档和注释中，提及 SoupRune 本身时应使用"框架"（或直接说"SoupRune"）。
> "引擎"一词保留给 Bevy、FRE 的规则引擎等真正属于引擎层的组件。
...
- `examples/` **不纳入** 800/500 门槛。它们是演示二进制，不是正式产品模块；可以保持灵活，但不能把正式复杂度偷偷塞进 example
  里
- 模块有**不同的职责** → 按职责拆分
...
- 不是放进 `core/` 目录就自动变成基础设施层了。如果一个模块需要知道 battle、overworld 或编辑器细节，它大概率就不该放在
  `core/`
- 一个简单判断规则是：`app_state/` 可以依赖 `core/`，但 `core/` 不应该反过来伸手依赖 `app_state/`
- 编辑器 crate 应该依赖**公开的框架 API**或**schema crate**，不要因为眼前方便就直接穿透到深层内部路径
...
下面这些 Bevy 相关规则，来自 Bevy 官方的插件开发指导和 `bevy_best_practices` 的思路；这里不照抄原文，而是用这个项目里能直接执行的人话写出来。
